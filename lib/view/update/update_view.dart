...
import 'package:gestionnairebloc/utils/extension/string.ext.dart';
  ...
  void update() {
    ...
    if (ws.isNotEmpty && ide.isNotEmpty && pwdStr.length >= 12) {
      if (widget.type == UpdateType.modify) {
        ...
        var pwd = Password(
            website: ws,
            identifier: EncryptData.encryptAES(ide),
            pwd: EncryptData.encryptAES(pwdStr));

        BlocProvider.of<MainBloc>(context).add(Create(pwd: pwd));
        context.go(Routes.home.path);
      }
    } else {
      var msg = (pwdStr.length < 12)
          ? "password must be bigger than 12 !"
          : "Something is empty !";
      ...
      child: SingleChildScrollView(
        child: Container(
          ...
          child: Column(
            ...
            children: [
              Text(
                "${widget.type.name.capitalize()} password",
              ...
              BlocBuilder<MainBloc, MainState>(
                builder: (context, mainState) {
                  if (widget.type == UpdateType.modify &&
                      mainState is ModifyErrorState) {
                    return Text(mainState.msg,
                        style: const TextStyle(color: Colors.red));
                  } else if (widget.type == UpdateType.add &&
                      mainState is AddErrorState) {
                    return Text(mainState.msg,
                        style: const TextStyle(color: Colors.red));
                  } else {
                    return Container();
